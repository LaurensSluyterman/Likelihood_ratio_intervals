...
model = xgb.XGBRegressor(objective='reg:squarederror', gamma=0.2, max_depth=6, reg_lambda=1, base_score=np.mean(Y))
model.fit(X, Y)

# Fit a second, perturbed, model
x_star = np.array([-1.2])
y_star = model.predict(x_star) + 0.5 / np.std(Y)
...
#%% Calculate the CIs
x_test_2 = np.linspace(-1.2, 1.2, 50)
CI = np.zeros((len(x_test_2), 2))
for i, x in enumerate(x_test_2):
    ci = CI_XGBx(model=model, x=x, X_train=X, Y_train=Y, n_steps=20, alpha=0.2, delta=0.5 / np.std(Y))
    CI[i, 0] = ci[0]
    CI[i, 1] = ci[1]
