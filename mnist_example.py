...
model.save('./models/CNNMnistmedium')
#%%


model = tf.keras.models.load_model('./models/CNNMnistmedium')
...
CI_classificationx(model=model, x=x_test_01[-1], X_train=x_train_01,
                   Y_train=y_train_01, p_hats=model.predict(x_train_01),
                   n_steps=150, alpha=0.1, n_epochs=3, fraction=0.1)

CI_classificationx(model=model, x=x_train_01[-1], X_train=x_train_01,
                   Y_train=y_train_01, p_hats=model.predict(x_train_01),
                   n_steps=150, alpha=0.1, n_epochs=3, fraction=0.1)
